  ...
  for (var deptName in fd){
    ...
    for (var s in dept["Stations"]){
      ...
      var position = codeAddress(station["Location"], function(c){
        var marker = new L.marker([c.lat, c.long]).addTo(map);
      });
    }
  }
}
var setCenterView = codeAddress("10550", function(c){
  var lat = c.lat;
  var long = c.long;
  var comma = ", ";
  var coor = lat + comma + long;
  console.log(coor);
});
var map = L.map("map").setView([40.9087956, -73.83308439999996], 14);
